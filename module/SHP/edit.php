...
$temp->setReplace("{content}", $temp->getTemplate("./template/SHP/SHP/edit.html"));
$HeadCode = "";
$selMV = 0 ;
$selBA = 0 ;
...
if($selMV !=""){
    ...
    $LighterNo = $BoatPlanHead->Ship_LighterNo;
}
...
if($CountItem >= 1 ){
    ...
    $lists = $lists . ' </div>';
    $lists = $lists . ' <div class="col-2 text-center col-in">';
    $lists = $lists . '     <input class="form-control" name="txtDescription[]"  readonly id="txtDescription[]" value="' .$Item->item . '" readonly> ';
    $lists = $lists . '  </div>';
    $lists = $lists . '   <div class="col-1 text-center col-in" style="padding-left:2px;">';
    $lists = $lists . '     <input class="form-control number" name="itemQTY[]" onkeypress="return floatOnly(event);" id="itemQTY[]" value="' . $Item->qty1 . '" readonly>';
    $lists = $lists . ' </div>';
