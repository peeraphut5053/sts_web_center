...
$temp = new ReplaceHtml("../template/MGT/print_tag_coil.html");
$cSql = new SqlSrv();
...
if ($printType == 0) { //==== new print 
    if (isset($_POST["coil_ids"])) {
        ...
        $g = array();
        for ($t = 0; $t < $ci_n; $t++) {
            $temp->setReplace("{printlist}", $temp->getTemplate("../template/MGT/tag_coil_block.html"));
        ...
        for ($t = 0; $t < $s_tag; $t++) {
            ...
            $temp->setReplace("{mdate}", "" . $POI->grn_hdr_date->format("Y-m-d") . "");      
            ...
            $val = $grn_nums[$t] . "!!" . $po_nums[$t] . "!!" . $lots[$t] . "!!" . $POI->item
                    ...
                    . "!!" . $coil_nos[$t] . "!!" . $POI->grn_hdr_date->format("Y-m-d") . "!!" . $thickss[$t] . "!!$tWidth!!" . 
