<?php
// [Used by sahathai_react_project - WorkCenter, Locations, Stop Machine Reasons & Packing Reports]
...
if ($load == "AddNewReasonFinishing") {
    ...
    $Trans = new JobOrder();
    $Trans->setConn($ConnSL);
    $Trans = $Trans->CreateNewReasonFinishing($reason_id, $time_stopped, $w_c,$remark);
    echo json_encode($Trans);
}
...
if ($load == "SelectFinishing") {
    ...
    $Trans = new JobOrder();
    $Trans->setConn($ConnSL);
    $Trans = $Trans->SelectFinishing($startdate, $enddate, $txtw_c,$type);
    echo json_encode($Trans);
}
