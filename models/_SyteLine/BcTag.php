...
order by [date]";
        ...
        $rs2 = $cSql->SqlQuery($this->StrConn, $query1);

        array_splice($rs, count($rs) - 1, 1);
        array_splice($rs1, count($rs1) - 1, 1);
        array_splice($rs2, count($rs2) - 1, 1);
        return array($rs, $rs1, $rs2);
    }
...
order by V_STS_PROD_TIME_REPORT_HOURLY.[date],V_STS_PROD_TIME_REPORT_HOURLY.wc";
    ...
    function getParetoDataFinishing($StartDate, $EndDate,$GroupBy) {
        $query = "select * from STS_finishing_reason where (time_stopped between '$StartDate' and '$EndDate') and w_c in ($GroupBy)"; ;
        $cSql = new SqlSrv();
        $rs = $cSql->SqlQuery($this->StrConn, $query);
        array_splice($rs, count($rs) - 1, 1);
        return array($rs);
