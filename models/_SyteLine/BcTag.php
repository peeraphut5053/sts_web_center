...
where tag.job is not null and line.doc_num = '$tag_id' and preship.do_num in ( " . $do_num_list . " ) ";
    ...
    Function SearchTagDetailCheckByDOBoat($tag_id, $do_num) {
        $cSql = new SqlSrv();
        $do_num_arr = explode(",", $do_num);
        $do_num_list = "";
        for ($i = 0; $i < count($do_num_arr); $i++) {
...
order by [date]";
        $query2 = "SELECT
    [date] = CONVERT(date, DayStart.[start_date]),
    DayStart.wc,
    [status] =
        CASE
            WHEN DayStart.end_date IS NOT NULL THEN 'red'
            ELSE
                CASE
                    WHEN MachineStop.[status] IS NULL THEN 'green'
                    ELSE MachineStop.[status]
                END
        END,
    reason_description =
        CASE
            WHEN MachineStop.[status] = 'yellow' THEN MachineStop.reason_description
            ELSE NULL
        END,
    time_stopped =
        CASE
            WHEN MachineStop.[status] = 'yellow' THEN MachineStop.time_stopped
            ELSE NULL
        END,
    totalStop = ISNULL(MachineStop.totalStop, 0),
    countStop = ISNULL(MachineStop.countStop, 0),
    matitem.size,

    operationSpeed =
        CASE
            WHEN DayStart.wc LIKE '%FM01' THEN fs.FM1
            WHEN DayStart.wc LIKE '%FM02' THEN fs.FM2
            WHEN DayStart.wc LIKE '%FM04' THEN fs.FM4
            WHEN DayStart.wc LIKE '%FM05' THEN fs.FM5
            WHEN DayStart.wc LIKE '%FM06' THEN fs.FM6
            WHEN DayStart.wc LIKE '%FM07' THEN fs.FM7
            WHEN DayStart.wc LIKE '%FM08' THEN fs.FM8
            WHEN DayStart.wc LIKE '%FM09' THEN fs.FM9
            WHEN DayStart.wc LIKE '%FM10' THEN fs.FM10
            WHEN DayStart.wc LIKE '%FM11' THEN fs.FM11
            WHEN DayStart.wc LIKE '%C%' THEN fs.FMC
            ELSE 0
        END,

    operationtime = ISNULL(fo.operationtime, 0),

    pcs_hr = ISNULL(matitem.pcs_hr, 0),
    multiplier = ISNULL(matitem.multiplier, 0)

FROM STS_machine_record DayStart
...
(
    ...
    (
        /* finishing */
        SELECT
            [date] = CONVERT(date, F.[time_stopped]),
            wc = F.w_c,
            [status] =
                CASE
                    WHEN F.time_end IS NULL OR F.down_time IS NULL THEN 'yellow'
                    ELSE 'green'
                END,
            reason_description = ISNULL(D.reason_description, '') + ' ' + ISNULL(F.remark, ''),
            time_stopped = CONVERT(time(0), F.[time_stopped]),
            sub.totalStop,
            sub.countStop,
            ROW_NUMBER() OVER (
                PARTITION BY CONVERT(date, F.[time_stopped]), F.w_c
                ORDER BY F.[time_stopped] DESC
            ) AS rowNo
        FROM STS_finishing_reason F
        INNER JOIN STS_finishing_reason_description D
            ON D.reason_id = F.reason_id
        INNER JOIN
        (
            SELECT
                w_c,
                [time_stopped] = CONVERT(date, [time_stopped]),
                totalStop = SUM(sub.down_time),
                countStop = COUNT(sub.down_time)
            FROM STS_finishing_reason sub
            WHERE CONVERT(date, sub.[time_stopped]) = CONVERT(date, GETDATE())
            GROUP BY w_c, CONVERT(date, [time_stopped])
        ) sub
            ON sub.[time_stopped] = CONVERT(date, F.[time_stopped])
           AND sub.w_c = F.w_c
        WHERE CONVERT(date, F.[time_stopped]) = CONVERT(date, GETDATE())
          AND CONVERT(time, F.[time_stopped]) < CONVERT(time, GETDATE())
        ...
        SELECT
            ...
            [status] =
                CASE
                    WHEN F.time_used IS NULL OR F.time_end IS NULL THEN 'yellow'
                    ELSE 'green'
                END,
            reason_description =
                ISNULL(D.reason_description, '') + ' ' +
                ISNULL(DD.[description], '') + ' ' +
                ISNULL(F.remark, ''),
            time_stopped = CONVERT(time(0), F.[time_stopped]),
            sub.totalStop,
            sub.countStop,
        ...
        FROM STS_forming_reason F
        INNER JOIN STS_forming_reason_description D
            ON D.reason_id = F.reason_id
        LEFT JOIN STS_forming_reason_description_detail DD
            ON DD.reason_detail_id = F.reason_detail_id
           AND DD.reason_id = F.reason_id
        INNER JOIN
        (
            SELECT
                w_c,
                [time_stopped] = CONVERT(date, [time_stopped]),
                totalStop = SUM(sub.time_used),
                countStop = COUNT(sub.time_used)
            FROM STS_forming_reason sub
            WHERE CONVERT(date, sub.[time_stopped]) = CONVERT(date, GETDATE())
            GROUP BY w_c, CONVERT(date, [time_stopped])
        ) sub
            ON sub.[time_stopped] = CONVERT(date, F.[time_stopped])
    ...
    ON MachineStop.[date] = CONVERT(date, DayStart.[start_date])
   AND MachineStop.wc = DayStart.wc
...
(
    ...
    (
        SELECT
            ...
            mat.wc,
            size =
                ISNULL(item.Uf_TypeEnd, '') + ' ' +
                ISNULL(item.uf_nps, '') + ' ' +
                ISNULL(item.uf_class, '') + ' ' +
                ISNULL(item.uf_grade, '') + ' x ' +
                ISNULL(item.uf_schedule, '') + ' x ' +
                ISNULL(item.uf_length, ''),
            mat.ref_num,
            ...
            (
                SELECT TOP 1
                    CASE
                        WHEN jr.run_basis_mch = 'P'
                             AND js.run_mch_hrs <> 0
                            THEN js.pcs_per_mch_hr
                        ELSE js.run_mch_hrs
                    END
                FROM item_mst item2
                INNER JOIN jobroute_mst jr
                    ON jr.job = item2.job
                   AND jr.suffix = item2.suffix
                INNER JOIN jrt_sch_mst js
                    ON js.job = item2.job
                   AND js.suffix = item2.suffix
                WHERE item2.item = mat.item
            ),

            multiplier =
                CASE
                    WHEN SUBSTRING(item.item, 22, 1) = 'M' THEN 1
                    WHEN SUBSTRING(item.item, 22, 1) = 'F' THEN 0.3048
                    ELSE 0
                END * CONVERT(decimal(10,4), ISNULL(item.uf_length_FT, 0)),

            ROW_NUMBER() OVER (
                PARTITION BY mat.wc
                ORDER BY mat.createdate DESC
            ) AS rowNo
        FROM matltran_mst mat
        INNER JOIN item_mst item
            ON mat.item = item.item
        INNER JOIN job_mst
            ON job_mst.job = mat.ref_num
           AND job_mst.item = mat.item
        WHERE CONVERT(date, mat.createdate) = CONVERT(date, GETDATE())
          AND mat.trans_type = 'F'
          AND mat.ref_type = 'J'
    ) mat
    WHERE mat.rowNo = 1
) matitem
    ON matitem.wc = DayStart.wc

LEFT JOIN STS_forming_operation fo
    ON matitem.ref_num = fo.job
   AND matitem.wc = fo.w_c
   AND matitem.item = fo.item

LEFT JOIN STS_forming_speed fs
    ON matitem.item LIKE '%' + fs.item + '%'
";
