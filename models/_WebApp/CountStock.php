...
class CountStock2 {
    ...
    function GetCountStockList() {
        $query = "Select * FROM STS_countstock_hdr";
        $cSql = new SqlSrv();
    ...
    function GetCountStockDetail($loc, $item_code, $id_hdr) {
        ...
        $query = "Select Convert(nvarchar,create_date,120) as conv_create_date,* FROM STS_countstock_detail where 1=1  and id_hdr = '$id_hdr' ";
    ...
    function CreateCountStockList($loc, $Uf_pack, $sum_pcs, $item_code, $qty_bundle, $qty_pcs) {
        ...
        $query = "INSERT INTO STS_countstock_hdr("
        ...
        for ($i = 0; $i < count($item_code); $i++) {
            $query2 = "INSERT INTO STS_countstock_detail("
                    . "id_detail, id_hdr, item_code,qty_bundle,qty_count_bundle, sum_qty_pcs,qty_pcs,qty_count_pcs,"
                    . "uf_pack, remark,create_date,approve_status,location"
                    . ")VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?) ";
            $params2 = array(
                "", $id_hdr, $item_code[$i], $qty_bundle[$i], 0, $sum_pcs[$i], $qty_pcs[$i], 0, $Uf_pack[$i], "", "", 0, $loc
            );
    ...
    function UpdateCountStockList($id_hdr, $item_code, $sum_qty_count_pcs, $qty_count_bundle, $qty_count_pcs, $remark) {
        $query = "update STS_countstock_detail set sum_qty_count_pcs='$sum_qty_count_pcs',qty_count_bundle = '$qty_count_bundle',qty_count_pcs='$qty_count_pcs',remark='$remark' where item_code= '$item_code' and id_hdr = '$id_hdr' ";
        $cSql = new SqlSrv();
    ...
    function chkStockApprove($loc) {
        $query = "Select * FROM STS_countstock_hdr where location = '$loc' and approve_status = '0' ";
        $cSql = new SqlSrv();
        $rs0 = $cSql->SqlQuery($this->StrConn, $query);
        array_splice($rs0, count($rs0) - 1, 1);
        if ($rs0) {
    ...
    function genLastIdhdr($loc) {
        $query = "Select * FROM STS_countstock_hdr where id_hdr like '%$loc%' order by id_hdr desc";
        $cSql = new SqlSrv();
        $rs0 = $cSql->SqlQuery($this->StrConn, $query);
        array_splice($rs0, count($rs0) - 1, 1);
        if ($rs0) {
    ...
    function confirmCheckStock($id_hdr) {
        $query2 = "update STS_countstock_hdr set approve_status='1' where id_hdr = '$id_hdr' ";
        $cSql2 = new SqlSrv();
        $rs2 = $cSql2->IsUpDel($this->StrConn, $query2);

        $query = "update STS_countstock_detail set approve_status='1' where id_hdr = '$id_hdr' ";
        $cSql = new SqlSrv();
        $rs0 = $cSql->IsUpDel($this->StrConn, $query);
        return $query;
    }
