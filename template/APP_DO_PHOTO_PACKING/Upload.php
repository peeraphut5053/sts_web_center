                ...
                if (is_array($queue_item) && isset($queue_item['name']) && isset($queue_item['tmp_name']) && isset($queue_item['new_name'])) {
                    ...
                    if ($move_result === true) {
                        ...
                        $this->move_uploaded_queue[$key] = array_merge(
                                $this->move_uploaded_queue[$key], array(
                        ...
                        $this->setErrorMessage(
                                sprintf(static::__('Unable to move uploaded file. (%s =&gt; %s)'), $queue_item['name'], $this->move_uploaded_to . DIRECTORY_SEPARATOR . $destination_name), 'RDU_MOVE_UPLOADED_FAILED', $queue_item['name'] . '=&gt; ' . $this->move_uploaded_to . DIRECTORY_SEPARATOR . $destination_name
                        );
                    }
        ...
        if (
            ...
            if (is_file($this->files[$this->input_file_name]['tmp_name'])) {
                ...
                if (stripos($file_content, '<?php') !== false || stripos($file_content, '<?=') !== false) {
                    ...
                    $this->setErrorMessage(
                            sprintf(static::__('Error! Found php embedded in the uploaded file. (%s).'), $this->files[$this->input_file_name]['name']), 'RDU_SEC_ERR_PHP', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                }
                ...
                if (stripos($file_content, '#!/') !== false && stripos($file_content, '/perl') !== false) {
                    ...
                    $this->setErrorMessage(
                            sprintf(static::__('Error! Found cgi/perl embedded in the uploaded file. (%s).'), $this->files[$this->input_file_name]['name']), 'RDU_SEC_ERR_CGI', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                }
                ...
                if (
                        stripos($file_content, '#!/') !== false && (stripos($file_content, '/bin/sh') !== false ||
                    ...
                    // found shell script.
                    $this->setErrorMessage(
                            sprintf(static::__('Error! Found shell script embedded in the uploaded file. (%s).'), $this->files[$this->input_file_name]['name']), 'RDU_SEC_ERR_CGI', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                }
...
// securityScan
    ...
    $error_messages, $code, $errorAttributes = '', $errorFileName = '', $errorFileSize = '', $errorFileMime = ''
    ) {
...
// setWebSafeFileName
    ...
    public function testGetUploadedMimetype($input_file_name = null) {
        ...
        if (
                !isset($_FILES[$input_file_name]['name']) || (isset($_FILES[$input_file_name]['name']) && $_FILES[$input_file_name]['name'] == null) ||
                !isset($_FILES[$input_file_name]['tmp_name']) || (isset($_FILES[$input_file_name]['tmp_name']) && $_FILES[$input_file_name]['tmp_name'] == null)
        ) {
...
// testGetUploadedMimetype
    ...
    public function upload() {
        ...
        if (!is_dir($this->move_uploaded_to)) {
            echo $this->move_uploaded_to;

            $this->setErrorMessage(
                    static::__('The target location where the uploaded file(s) will be moved to is not folder or directory.'), 'RDU_MOVE_UPLOADED_TO_NOT_DIR', $this->move_uploaded_to
            );
            return false;
        } elseif (is_dir($this->move_uploaded_to) && !is_writable($this->move_uploaded_to)) {
            $this->setErrorMessage(
                    static::__('The target location where the uploaded file(s) will be moved to is not writable. Please check the folder permission.'), 'RDU_MOVE_UPLOADED_TO_NOT_WRITABLE', $this->move_uploaded_to
            );
            return false;
        } else {
    ...
    protected function uploadSingleFile() {
        ...
        if (is_array($this->files[$this->input_file_name]) && array_key_exists('error', $this->files[$this->input_file_name]) && $this->files[$this->input_file_name]['error'] != 0) {
            switch ($this->files[$this->input_file_name]['error']) {
                case 1:
                    $this->setErrorMessage(
                            sprintf(static::__('The uploaded file exceeds the max file size directive. (%s &gt; %s).'), $this->files[$this->input_file_name]['size'], ini_get('upload_max_filesize')), 'RDU_' . $this->files[$this->input_file_name]['error'], $this->files[$this->input_file_name]['size'] . ' &gt; ' . ini_get('upload_max_filesize'), $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                case 2:
                    $this->setErrorMessage(
                            static::__('The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form.'), 'RDU_' . $this->files[$this->input_file_name]['error'], '', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                case 3:
                    $this->setErrorMessage(
                            static::__('The uploaded file was only partially uploaded.'), 'RDU_' . $this->files[$this->input_file_name]['error'], '', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                case 4:
                    $this->setErrorMessage(
                            static::__('You did not upload the file.'), 'RDU_' . $this->files[$this->input_file_name]['error'], '', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                case 6:
                    $this->setErrorMessage(
                            static::__('Missing a temporary folder.'), 'RDU_' . $this->files[$this->input_file_name]['error'], '', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                case 7:
                    $this->setErrorMessage(
                            static::__('Failed to write file to disk.'), 'RDU_' . $this->files[$this->input_file_name]['error'], '', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                case 8:
                    $this->setErrorMessage(
                            static::__('A PHP extension stopped the file upload.'), 'RDU_' . $this->files[$this->input_file_name]['error'], '', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
            }
        }
        ...
        if (
                empty($this->files[$this->input_file_name]) || (is_array($this->files[$this->input_file_name]) &&
                array_key_exists('name', $this->files[$this->input_file_name]) &&
                $this->files[$this->input_file_name]['name'] == null) || (is_array($this->files[$this->input_file_name]) &&
                array_key_exists('tmp_name', $this->files[$this->input_file_name]) &&
                $this->files[$this->input_file_name]['tmp_name'] == null)
        ) {
            $this->setErrorMessage(
                    static::__('You did not upload the file.'), 'RDU_4', '', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
            );
            return false;
        }
...
// uploadSingleFile
    ...
    protected function validateExtensionAndMimeType() {
        ...
        if (!is_array($file_name_explode)) {
            unset($file_name_explode);
            $this->setErrorMessage(
                    sprintf(static::__('Unable to validate extension for the file %s.'), $this->files[$this->input_file_name]['name']), 'RDU_UNABLE_VALIDATE_EXT', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
            );
            return false;
        }
        ...
        if (is_array($this->allowed_file_extensions) && !in_array($file_extension, $this->allowed_file_extensions)) {
            ...
            $this->setErrorMessage(
                    sprintf(static::__('You have uploaded the file that is not allowed extension. (%s)'), $this->files[$this->input_file_name]['name']), 'RDU_NOT_ALLOW_EXT', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
            );
            return false;
        }
        ...
        if (is_array($this->file_extensions_mime_types) && !empty($this->file_extensions_mime_types)) {
            if (!array_key_exists($file_extension, $this->file_extensions_mime_types)) {
                unset($file_extension);
                $this->setErrorMessage(
                        sprintf(static::__('Unable to validate the file extension and mime type. (%s). This file extension was not set in the &quot;file_extensions_mime_types&quot; property.'), $this->files[$this->input_file_name]['name']), 'RDU_UNABLE_VALIDATE_EXT_AND_MIME', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                );
                return false;
            } else {
                $Finfo = new \finfo();
                ...
                if (is_array($this->file_extensions_mime_types[$file_extension]) && !in_array(strtolower($file_mimetype), array_map('strtolower', $this->file_extensions_mime_types[$file_extension]))) {
                    ...
                    $this->setErrorMessage(
                            sprintf(static::__('The uploaded file has invalid mime type. (%s : %s).'), $this->files[$this->input_file_name]['name'], $file_mimetype), 'RDU_INVALID_MIME', $this->files[$this->input_file_name]['name'] . ' : ' . $file_mimetype, $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $file_mimetype
                    );
                    unset($file_mimetype);
                    return false;
                    ...
                    $this->setErrorMessage(
                            static::__('Unable to validate mime type. The format of &quot;file_extensions_mime_types&quot; property is incorrect.'), 'RDU_UNABLE_VALIDATE_MIME', '', $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                }
...
// validateExtensionAndMimeType
    ...
    protected function validateFileSize() {
        ...
        if (is_array($this->files[$this->input_file_name]) && array_key_exists('size', $this->files[$this->input_file_name]) && $this->files[$this->input_file_name]['size'] > $this->max_file_size) {
            $this->setErrorMessage(
                    sprintf(static::__('The uploaded file exceeds the max file size directive. (%s &gt; %s).'), $this->files[$this->input_file_name]['size'], $this->max_file_size), 'RDU_1', $this->files[$this->input_file_name]['size'] . ' &gt; ' . $this->max_file_size, $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
            );
            return false;
        } else {
        ...
        if (
            ...
            if ($image === false) {
                ...
                if (
                    ...
                    $this->setErrorMessage(
                            sprintf(static::__('The uploaded image contain no image or multiple images. (%s).'), $image[0] . 'x' . $image[1]), 'RDU_IMG_NO_OR_MULTIPLE_IMAGES', $image[0] . 'x' . $image[1], $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                } else {
                    // if image dimensions are larger than max.
                    $this->setErrorMessage(
                            sprintf(static::__('The uploaded image dimensions are larger than allowed max dimensions. (%s &gt; %s).'), $image[0] . 'x' . $image[1], $this->max_image_dimensions[0] . 'x' . $this->max_image_dimensions[1]), 'RDU_IMG_DIMENSION_OVER_MAX', $image[0] . 'x' . $image[1] . ' &gt; ' . $this->max_image_dimensions[0] . 'x' . $this->max_image_dimensions[1], $this->files[$this->input_file_name]['name'], $this->files[$this->input_file_name]['size'], $this->files[$this->input_file_name]['type']
                    );
                    return false;
                }
            } else {
...
// validateImageDimension
    ...
    protected function validateOptionsProperties() {
        ...
        if (
                !is_array($this->max_image_dimensions) || (is_array($this->max_image_dimensions) && (count($this->max_image_dimensions) != 2 || (count($this->max_image_dimensions) == 2 &&
...
if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
    ...
    $Upload->move_uploaded_to = './APP_DO_PHOTO_PACKING_img';
    // อนุญาตไฟล์ gif, jpg, png
    $Upload->allowed_file_extensions = array('gif', 'jpg', 'jpeg', 'png');
    // ขนาดไฟล์สูงสุดคือ 900KB.
    $Upload->max_file_size = 9999999999900000;
    // คุณสามารถตั้งชื่อไฟล์ที่อัพโหลดใหม่ได้หรือปล่อยว่างไว้เพื่อใช้ชื่อเดิม. กำหนดชื่อเท่านั้น ไม่รวมนามสกุลไฟล์.
//    $Upload->new_file_name = "sts_".date("Ymd")."_".$_POST['tag_id'];
    print_r($_POST);
    $Upload->new_file_name = "tag_" .$_POST['tag_id'] ."_".$_POST['img_upload_file_name'];
    //file name = tag_id-1,tag-id-2
    //ID,tag_id,filename[img1,img2],create_date,create_by
